      ...
      <ul class="nav nav-tabs search-nav-tabs" id="myTab" role="tablist">
        ...
        <div class="col-md-2 col-sm-14">
          <button
            @click="goToSearch"
            type="submit"
            class="btn btn-primary btn-search col-sm-14"
          >
            Tìm việc làm
          </button>
        </div>
...
const emit = defineEmits(["update-urgent-jobs"]);
...
const handleSearch = async () => {
  try {
    ...
    if (searchQuery.value.company)
      params.append("keyword", searchQuery.value.company);
    ...
    if (res.data.success) {
      emit("update-urgent-jobs", res.data.data);
    }
  } catch (error) {
